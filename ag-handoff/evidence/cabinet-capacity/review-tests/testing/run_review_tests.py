#!/usr/bin/env python3
"""Offline Lua 5.4 function tests. NOT Project Zomboid or a network/server test."""
...
ROOT = Path(sys.argv[1]) if len(sys.argv) > 1 else HERE.parent.parent / 'original'
RUNTIME = ROOT / 'source/server_runtime'
libname = ctypes.util.find_library('lua5.4') or ctypes.util.find_library('lua-5.4')
if not libname:
    raise SystemExit('Requires a local Lua 5.4 shared library; nothing was installed.')
lib = C.CDLL(libname)
...
L = lib.luaL_newstate()
if not L: raise SystemExit('Unable to initialize Lua state')
lib.luaL_openlibs(L)

def run(text: str, label: str) -> None:
    data=text.encode('utf-8')
    code=lib.luaL_loadbufferx(L,data,len(data),label.encode(),b't')
    if code==0: code=lib.lua_pcallk(L,0,0,0,0,None)
    if code:
...
try:
    run((HERE/'fixtures.lua').read_text(), 'fixtures')
    ...
    run((HERE/'candidate_logic.lua').read_text(), 'candidate logic only')
    run((HERE/'cases.lua').read_text(), 'review cases')
finally:
    lib.lua_close(L)
