ReviewCase('01_damage_init_still_leaves_both_cabinets_at_20',function()
...
ReviewCase('02_candidate_access_restores_capacity_after_identity_checks',function()
    local v,ch=ReviewFixture({'Base.Mov_ModernCounter'})
    ReviewEq(VLS.ContainerAccess.UniversalSlot(v,v.parts[1],ch),true)
    ReviewEq(v.parts[1].container.capacity,50)
end)
ReviewCase('03_client_refresh_repairs_local_copy_only',function()
...
ReviewCase('05_update_restores_with_matching_item_and_profile_ids',function()
    local v=ReviewFixture({'Base.Mov_ModernCounter'})
    local p=v.parts[1]
...
ReviewCase('07_candidate_access_rejects_unauthorized_access',function()
    local v,ch=ReviewFixture({'Base.Mov_ModernCounter'})
    local p=v.parts[1]
    VLS.Init.UniversalSlot(v,p)
    p.container.capacity=20
    ReviewEq(VLS.ContainerAccess.UniversalSlot(v,p,ch),true)
    ReviewEq(p.container.capacity,50)
    p.container.capacity=20
    ch.vehicle=nil
    ReviewEq(VLS.ContainerAccess.UniversalSlot(v,p,ch),false)
...
print('RESULT 8/8 candidate offline function checks passed; no runtime failure/reload/transfer was captured')
